...
class ItemControllerTest {
	...
	void findItems() throws Exception {
		...
		mockMvc.perform(RestDocumentationRequestBuilders.get("/items/{id}", requestId))
			.andExpect(MockMvcResultMatchers.status().isOk())
			.andDo(MockMvcRestDocumentation.document("item",
				pathParameters(
					parameterWithName("id").description("조회할 아이템 ID")
				),
				responseFields(
					...
					fieldWithPath("createdAt").description("생성 시간 (ISO 8601)")
				)
			));
	}
